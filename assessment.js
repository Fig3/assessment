...
// 無名関数 関数名を省略して直接書いただけ
assessmentButtun.onclick = () =>{
    ...
    // 診断結果表示エリアの作成
    removeAllChildren(resultDivided);// 子要素を全削除
    ...
    anchor.setAttribute('data-via', 'gifjpegpng');
...
function assessment(userName){
    ...
    for(let i=0; i<userName.length; i++){
        ansSumId += userName.charCodeAt(i);
    }
    const ansId = ansSumId % answers.length;
    let result = answers[ansId];
    result = result.replace(/\{userName\}/g, userName);
    return result;
}

console.log(assessment('太郎'));
console.log(assessment('yosami'));
console.log(assessment('太郎'));
